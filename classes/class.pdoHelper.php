...
class pdoHelper
{
    ...
    public static function getInstance(): ?pdoHelper
    {
    ...
    {
        $this->pdo = new PDO("mysql:host=" . self::$host . ";dbname=" . self::$dataBaseName . ";charset=" . self::$charset,
    ...
    public function insert($table, $vars, $values = [], $needLastInsertId = 0, $ignore = 0)
    {
        ...
        if (isset($values[0])) {
            ...
            $stmt = $this->pdo->prepare("INSERT$ignore INTO $table (`" . implode("`,`",
                    $vars) . "`) VALUES " . implode(",", $valuesStructure) . ";");
        } else {
            $allow_values = array();
     ...
     * @return array
     */
    public function selectRows($query, $values = [], $noNeedEmulation = 0, $unique = 0): ?array
    {
        if ($noNeedEmulation) {
     ...
     * @return mixed
     */
    public function selectVar($query, $values = [])
    {
        $stmt = $this->pdo->prepare($query);
        $stmt->execute($values);
     ...
     * @return array
     */
    public function selectRowsVar($query, $values = [], $var = "", $noNeedEmulation = 0, $unique = 0): ?array
    {
        if ($noNeedEmulation) {
            $this->pdo->setAttribute(PDO::ATTR_EMULATE_PREPARES, false);
        }
    ...
    public function count($query, $values = []): int
    {
        $stmt = $this->pdo->prepare($query);
        $stmt->execute($values);
     ...
     */
    public function getStmt(string $query, $values = [], $no_need_buffering = 0)
    {
     ...
     * @return string
     */
    static function set_placeholder(array $values): string
    {
